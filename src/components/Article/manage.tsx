		...
		return (
			<div className="flex pdb10 pdt10">
				<div className="flex flex30">
					...
					<DropdownButton id="dropdown-basic-button" variant="warning" title="批量操作">
						<Dropdown.Item onClick={(e: any) => this.handleBatch("batchPublish", e)}>发布</Dropdown.Item>
						<Dropdown.Item onClick={(e: any) => this.handleBatch("batchDelete", e)}>删除</Dropdown.Item>
					</DropdownButton>
				</div>
		...
		return (
			<tbody>
				{data.map((it: any) => (
					<tr key={it._id}>
						<td>
							<Form.Check type="checkbox" label={it.id} onChange={(e: any) => this.handleChecked(it._id, e)} />
						</td>
